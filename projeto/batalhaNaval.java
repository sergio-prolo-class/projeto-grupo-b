...
public class batalhaNaval {
    ...
    // static int numL = random.nextInt(10); //numero aleatório entre 0 e 9 para
    // linha
    // static int numC = random.nextInt(10); //numero aleatório entre 0 e 9 para
    // coluna
    // static boolean Vertical = random.nextBoolean(); //booleano aleatório para
    // orientação do barco
    // System.out.println(posiçãoL);
    // System.out.println(posiçãoC);
    // System.out.println(Vertical);
    // Arranjo do tabuleiro como varável estática, para ser atualizado por todos os
    // métodos
    ...
    ////////////////////////////////////// MÉTODOS ESTÁTICOS PARA GERAÇÃO DE
    /// TABULEIRO //////////////////////////////////////

    ////////////////////////////////////// para verificar se a peça cabe no
    /// tabuleiro //////////////////////////////////////

    static boolean barcoCabe(boolean Vertical, int numL, int numC, int tamanhoBarco) {

        if (Vertical) {
            // para linha aleatória, a altura da linha tem espaço necessario pro barco no
            // tabuleiro? true or false
            boolean barcoCabe = ((numL + tamanhoBarco) <= tamanhoTabuleiro) ? true : false;
            return barcoCabe;
        } else {
            // para coluna aleatória, a posição da coluna tem espaço necessario pro barco no
            // tabuleiro? true or false
    ...
    ////////////////////////////////////// para verificar se o tabuleiro está
    /// vago //////////////////////////////////////
    ...
    static boolean tabuleiroVago(boolean Vertical, int numL, int numC, int tamanhoBarco) {
        ...
        if (Vertical) { // se vertical verdadeiro,
            for (int i = 0; i < tamanhoBarco; i++) { // enquanto i < tamanho do barco,
                if (!tabuleiro[numL + i][numC].equals(vazio)) { // a posição do tabuleiro [random linha + i][coluna
                                                                // fixa] é diferente do conteudo de vazio?
                    return false; // retorne tabuleiroVago
                }
            } // equals para comparar arranjo com conteudo de string
        } else { // senão
            for (int i = 0; i < tamanhoBarco; i++) { // enquanto i < tamanho do barco,
                if (!tabuleiro[numL][numC + i].equals(vazio)) { // a posição do tabuleiro [random linha + i][coluna
                                                                // fixa] é diferente do conteudo de vazio?
                    return false; // retorne tabuleiroVago
                }
            }
    ...
    static void colocarBarco(int tamanhoBarco, String simbolo) {
        ...
        while (!barcoColocado) { // enquanto o barco não estiver colocado, continue tentando
            ...
            if (barcoCabe(vertical, numL, numC, tamanhoBarco)
                    ...
                    tabuleiroVago(vertical, numL, numC, tamanhoBarco)) { // de orientação, linha, coluna e tamanho do
                                                                         // barco

                for (int i = 0; i < tamanhoBarco; i++) { // para i = posição, enquanto i < tamanho do barco, i++
                    if (vertical) { // se vertical for verdadeiro
                        tabuleiro[numL + i][numC] = simbolo; // atualize a posição do tabuleiro [linha + i][coluna fixa]
                                                             // com o símbolo do barco
                    } else { // senão (horizontal)
                        tabuleiro[numL][numC + i] = simbolo; // atualize a posição do tabuleiro [linha fixa][coluna + i]
                                                             // com o símbolo do barco
                    }
                }
    ...
    ////////////////////////////////////// MÉTODOS ESTÁTICOS PARA VALIDAÇÃO DE
    /// TABULEIRO //////////////////////////////////////
     ...
     */
    static void dfs(boolean[][] pos, boolean[][] vis, int l, int c) {
        int[] dl = { -1, 1, 0, 0 };
        int[] dc = { 0, 0, -1, 1 };

        ArrayDeque<int[]> stack = new ArrayDeque<>();
        stack.push(new int[] { l, c });
        vis[l][c] = true;
        ...
        while (!stack.isEmpty()) {
            ...
            for (int k = 0; k < 4; k++) {
                ...
                if (nl >= 0 && nl < 10 && nc >= 0 && nc < 10) {
                    if (pos[nl][nc] && !vis[nl][nc]) {
                        vis[nl][nc] = true;
                        stack.push(new int[] { nl, nc });
                    }
                }
            }
        }
    ...
    static boolean validarFormatoNavio(String simbolo, int tamanhoEsperado) {
        ...
        int minL = 10, maxL = -1, minC = 10, maxC = -1;
        ...
        // 2) diagonal REAL
        boolean mesmaLinha = (minL == maxL);
        boolean mesmaColuna = (minC == maxC);

        // diagonal real: duas células do navio encostadas só pela diagonal
        for (int l = 0; l < 9; l++) {
            for (int c = 0; c < 9; c++) {

                // caso 1: diagonal principal
                if (pos[l][c] && pos[l + 1][c + 1]) {
                    if (!pos[l + 1][c] && !pos[l][c + 1]) {
                        System.out.println("INVÁLIDO (navio na diagonal)");
                        return false;
                    }
                }

                // caso 2: diagonal secundária
                if (pos[l + 1][c] && pos[l][c + 1]) {
                    if (!pos[l][c] && !pos[l + 1][c + 1]) {
                        System.out.println("INVÁLIDO (navio na diagonal)");
                        return false;
        ...
        // Se tem mais de um componente, precisamos ver o que separa
        if (componentes > 1) {
            boolean separaPorAgua = false;
            boolean separaPorOutroNavio = false;
        ...
        if (count != tamanhoEsperado) {
            System.out.println("INVÁLIDO (tamanho incorreto de navio)");
            return false;
        }
    ...
    // if (args.length == 0 || !args[0].equals("G")) { //equals para comparar
    // arranjo com conteudo de string
    ...
    // for (int i = 0; i < tamanhoBarco.length; i++) { //para i = 0; enquanto i < o
    // numero de barcos, i++
    // colocarBarco(tamanhoBarco[i], simbolos[i]); //coloca o barco no tabuleiro,
    // tamanho e símbolo do barco correspondente
    // }
